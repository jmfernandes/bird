...
handleOpenChannel(mosiop_t iop, PhidgetNetConnHandle nc, netreq_t *req) {
	...
	if (def == NULL)
		return (sendSimpleReply(nc, req->nr_reqseq, EPHIDGET_INVALIDARG, "Invalid Channel Class."));

	// NOTE - We are no longer enforcing client/server class version match
	// XXX - remove once happy
	//if (def->version != version) {
	//	netlogerr("received open request for channel class 0x%x, with version %d; expected version %d",
	//	  cclass, version, def->version);
	//	return (sendSimpleReply(nc, req->nr_reqseq, EPHIDGET_BADVERSION, "NetworkServer class version mismatch. Make sure client and server are running the same library version."));
	//}
	...
	if (res != EPHIDGET_OK) {
		...
		res = sendSimpleReply(nc, req->nr_reqseq, res, strlen(errbuf) > 0 ? errbuf : NULL);
