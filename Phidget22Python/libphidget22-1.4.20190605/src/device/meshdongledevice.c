...
processPacket(PhidgetMeshDongleDeviceHandle phid) {
	...
	if (index == VINTMESH_INPACKET_DONGLE_INDEX) {
		...
		len = phid->packetBuf[0];
		assert(len <= MAX_IN_PACKET_SIZE);
...
PhidgetMeshDongleDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetMeshDongleDeviceHandle phid = (PhidgetMeshDongleDeviceHandle)ch->parent;

	assert(phid->phid.deviceInfo.class == PHIDCLASS_MESHDONGLE);
