...
static double lookup_temperature(double, PhidgetTemperatureSensor_ThermocoupleType);
static double lookup_voltage(double, PhidgetTemperatureSensor_ThermocoupleType);
 ...
 * These ranges are "Maximum Useful Temperature Range" specified here:
...
PhidgetTemperatureSensorDevice_initAfterOpen(PhidgetDeviceHandle device) {
	PhidgetTemperatureSensorDeviceHandle phid;
	int i;

	assert(device);

	phid = (PhidgetTemperatureSensorDeviceHandle)device;
	phid->ambientSensorIndex = phid->devChannelCnts.numTempInputs - 1;

	//Setup max/min values
	switch (phid->phid.deviceInfo.UDD->uid) {
	case PHIDUID_1051_OLD:
		phid->thermocoupleType[0] = PUNK_ENUM;
		phid->maxTemperature[0] = PUNK_DBL;
		phid->minTemperature[0] = PUNK_DBL;
		phid->maxTemperature[phid->ambientSensorIndex] = 125;
		phid->minTemperature[phid->ambientSensorIndex] = -40;
		phid->interruptRate = 32;
		break;
	...
	case PHIDUID_1051:
		...
		phid->maxTemperature[phid->ambientSensorIndex] = 125;
		phid->minTemperature[phid->ambientSensorIndex] = -40;
		phid->potentialMax = round_double((((5 / GAIN)) + OFFSET_200), 5);
		phid->potentialMin = round_double(((((5 - (65535.0 / 13104.0)) / GAIN)) + OFFSET_200), 5);
		phid->interruptRate = 32;
		break;

	case PHIDUID_1051_AD22100:
		phid->thermocoupleType[0] = PUNK_ENUM;
		phid->maxTemperature[0] = PUNK_DBL;
		phid->minTemperature[0] = PUNK_DBL;
		phid->maxTemperature[phid->ambientSensorIndex] = 150;
		phid->minTemperature[phid->ambientSensorIndex] = -50;
		phid->potentialMax = round_double((-((-OFFSET_300) / GAIN)), 5);
		phid->potentialMin = round_double((-(((65535.0 / 13104.0) - OFFSET_300) / GAIN)), 5);
		phid->interruptRate = 32;
		break;

	case PHIDUID_1051_TERMINAL_BLOCKS:
		phid->thermocoupleType[0] = PUNK_ENUM;
		phid->maxTemperature[0] = PUNK_DBL;
		phid->minTemperature[0] = PUNK_DBL;
		phid->maxTemperature[phid->ambientSensorIndex] = 150;
		phid->minTemperature[phid->ambientSensorIndex] = -55;
		phid->potentialMin = round_double((((0.23 - PHIDID_1048_OFFSET) / PHIDID_1048_GAIN)), 5);
		phid->potentialMax = round_double((((3.846 - PHIDID_1048_OFFSET) / PHIDID_1048_GAIN)), 5);
		phid->interruptRate = 32;
		break;

	case PHIDUID_1048:
		for (i = 0; i < phid->devChannelCnts.numTempInputs - 1; i++) {
			phid->thermocoupleType[i] = PUNK_ENUM;
			phid->maxTemperature[i] = PUNK_DBL;
			phid->minTemperature[i] = PUNK_DBL;
		}
		phid->maxTemperature[phid->ambientSensorIndex] = 150;
		phid->minTemperature[phid->ambientSensorIndex] = -55;
		phid->potentialMin = round_double((((0.23 - PHIDID_1048_OFFSET) / PHIDID_1048_GAIN)), 5);
	...
	case PHIDUID_1045:
		phid->maxTemperature[0] = 380;
		phid->minTemperature[0] = -70;
		phid->maxTemperature[phid->ambientSensorIndex] = 125;
		phid->minTemperature[phid->ambientSensorIndex] = -40;
		phid->interruptRate = 32;
		break;
	...
	for (i = 0; i < phid->devChannelCnts.numTempInputs; i++) {
		phid->temperature[i] = PUNK_DBL;
		phid->lastTrigger[i] = PUNK_DBL;
		phid->lastVoltageTrigger[i] = PUNK_DBL;
		phid->temperatureChangeTrigger[i] = 0.1;
	}
	for (i = 0; i < phid->devChannelCnts.numVoltageInputs; i++) {
		phid->voltage[i] = PUNK_DBL;
		phid->voltageChangeTrigger[i] = 0.005;
	}
	phid->lastAmbientTrigger = PUNK_DBL;
...
#define CALLTM	(phid->interruptRate * phid->di_callcnt)

static PhidgetReturnCode CCONV
PhidgetTemperatureSensorDevice_dataInput(PhidgetDeviceHandle device, uint8_t *buffer, size_t length) {
	double temperature[TEMPSENSOR_MAXSENSORS] = { 0 };
	double potential[TEMPSENSOR_MAXSENSORS] = { 0 };
	PhidgetTemperatureSensorDeviceHandle phid;
	double Vad, Vthermocouple, VtempIC, Temp;
	...
	case PHIDUID_1051_OLD:
		/* OLD version */
		ambientTemperature = (double)((int16_t)buffer[2]);
		temperature[0] = (double)((int16_t)((uint16_t)buffer[0] + ((uint16_t)buffer[1] << 8)));
		break;
	...
	case PHIDUID_1051:
		...
		if (buffer[3] & 0x30)
			buffer[3] |= 0xf0;
		ambientTemperature = (double)((int16_t)((uint16_t)buffer[2] + ((uint16_t)buffer[3] << 8))) / 64.0;
		Vad = ((double)((uint16_t)buffer[0] + ((uint16_t)buffer[1] << 8))) / 13104.0;

		potential[0] = round_double(((((5 - Vad) / GAIN)) + OFFSET_200), 5);
	...
	case PHIDUID_1051_AD22100:
		...
		VtempIC = ((double)((uint16_t)buffer[2] + ((uint16_t)buffer[3] << 8))) / 13104.0;
		ambientTemperature = round_double(((VtempIC - 1.375) / 0.0225), 3);

		//Thermocouple
		Vad = ((double)((uint16_t)buffer[0] + ((uint16_t)buffer[1] << 8))) / 13104.0;
		potential[0] = round_double((-((Vad - OFFSET_300) / GAIN)), 5);
	...
	case PHIDUID_1051_TERMINAL_BLOCKS:
		//Temp IC - absolute possible range: -61.1 - 161.16
		ambientTemperature = round_double(((int16_t)(((int16_t)buffer[2] << 8) + (buffer[3])) / 128.0), 4);

		//Thermocouple
		Vad = ((double)((uint16_t)buffer[1] + ((uint16_t)buffer[0] << 8))) / 16000.0;
		potential[0] = round_double((((Vad - PHIDID_1048_OFFSET) / PHIDID_1048_GAIN)), 5);
	...
	case PHIDUID_1048:
		//Temp IC - absolute possible range: -61.1 - 161.16
		ambientTemperature = round_double(((int16_t)(((int16_t)buffer[8] << 8) + (buffer[9])) / 128.0), 4);
		// Don't include the ambient sensor in the loop
		for (i = 0; i < phid->devChannelCnts.numTempInputs - 1; i++) {
			//Thermocouple
			Vad = ((double)((uint16_t)buffer[1 + 2 * i] + ((uint16_t)buffer[0 + 2 * i] << 8))) / 16000.0;
			potential[i] = round_double((((Vad - PHIDID_1048_OFFSET) / PHIDID_1048_GAIN)), 7);
			...
			//logdebug("Index: %d Val: 0x%08x Potential: %0.6lf", i,
			//  ((uint16_t)buffer[1+2*i]+((uint16_t)buffer[0+2*i]<<8)), potential[i]);
		}
		break;

	case PHIDUID_1045:
		//Temp IC - absolute possible range: -61.1 - 161.16
		ambientTemperature = round_double(((((int16_t)buffer[3] << 8) + buffer[2]) * 0.02) - 273.15, 2);
		temperature[0] = round_double(((((int16_t)buffer[1] << 8) + buffer[0]) * 0.02) - 273.15, 2);
		break;
	...
	if (ambientTemperature > phid->maxTemperature[phid->ambientSensorIndex]) {
		phid->temperature[phid->ambientSensorIndex] = PUNK_DBL;
		ch = getChannel(phid, phid->devChannelCnts.numTempInputs + phid->devChannelCnts.numVoltageInputs);
		if (ch != NULL) {
			SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
			  "Ambient sensor reported a temperature of %.4fC which is over the maximum of %.4fC.",
			  ambientTemperature, phid->maxTemperature[phid->ambientSensorIndex]);
			PhidgetRelease(&ch);
		}
	} else if (ambientTemperature < phid->minTemperature[phid->ambientSensorIndex]) {
		phid->temperature[phid->ambientSensorIndex] = PUNK_DBL;
		ch = getChannel(phid, phid->devChannelCnts.numTempInputs + phid->devChannelCnts.numVoltageInputs);
		if (ch != NULL) {
			SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
			  "Ambient sensor reported a temperature of %.4fC which is below the minimum of %.4fC.",
			  ambientTemperature, phid->minTemperature[phid->ambientSensorIndex]);
			PhidgetRelease(&ch);
		}
	} else {
		phid->temperature[phid->ambientSensorIndex] = ambientTemperature;
		...
		case PHIDUID_1048:
			...
			// Don't include the ambient sensor in the loop
			for (i = 0; i < phid->devChannelCnts.numTempInputs - 1; i++) {
				if (phid->thermocoupleType[i] == (PhidgetTemperatureSensor_ThermocoupleType)PUNK_ENUM)
					continue;
				v = lookup_voltage(thermocouple_useful_range[phid->thermocoupleType[i]][1] - phid->temperature[phid->ambientSensorIndex],
					  phid->thermocoupleType[i]);
				if (v > phid->potentialMax)
					phid->maxTemperature[i] = (int)((lookup_temperature(phid->potentialMax,
						phid->thermocoupleType[i]) + phid->temperature[phid->ambientSensorIndex]) / 10) * 10;

				v = lookup_voltage(thermocouple_useful_range[phid->thermocoupleType[i]][0] - phid->temperature[phid->ambientSensorIndex],
				  phid->thermocoupleType[i]);
				if (v < phid->potentialMin)
					phid->minTemperature[i] = (int)((lookup_temperature(phid->potentialMin,
						phid->thermocoupleType[i]) + phid->temperature[phid->ambientSensorIndex]) / 10) * 10;
			}
			break;
	...
	// Don't include the ambient sensor in the loop
	for (i = 0; i < phid->devChannelCnts.numTempInputs - 1; i++) {
		ch = getChannel(phid, i);
		// If ambient temperature was outside of allowed range, then calculated temperature will be wrong
		if (phid->temperature[phid->ambientSensorIndex] == PUNK_DBL) {
			phid->temperature[i] = PUNK_DBL;
			if (ch != NULL) {
				SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
				  "Temperature cannot be calculated because the ambient temperature is unknown.");
			}
		} else if (temperature[i] == PUNK_DBL) {
			phid->temperature[i] = PUNK_DBL;
			if (phid->phid.deviceInfo.UDD->uid == PHIDUID_1045 ||
			  phid->thermocoupleType[i] != (PhidgetTemperatureSensor_ThermocoupleType)PUNK_ENUM) {
				if (ch != NULL) {
					SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
					  "Input reported an undefined temperature. Probably there is no thermocouple plugged in.");
				}
			}
		} else if (temperature[i] > phid->maxTemperature[i]) {
			phid->temperature[i] = PUNK_DBL;
			if (ch != NULL) {
				SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
				  "Input reported a temperature of %.4fC which is above the maximum of %.4fC.", temperature[i], phid->maxTemperature[i]);
			}
		} else if (temperature[i] < phid->minTemperature[i]) {
			phid->temperature[i] = PUNK_DBL;
			if (ch != NULL) {
				SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
					"Input reported a temperature of %.4fC which is below the minimum of %.4fC.", temperature[i], phid->minTemperature[i]);
			}
		} else {
		...
		case PHIDUID_1048:
			if (potential[i] > phid->potentialMax) {
				phid->voltage[i] = PUNK_DBL;
				if ((ch = getChannel(phid, i + phid->devChannelCnts.numTempInputs)) != NULL) {
					SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
					  "Input reported a potential of %.4fmV which is above the maximum of %.4fmV.", potential[i], phid->potentialMax);
					PhidgetRelease(&ch);
				}
				...
				if ((ch = getChannel(phid, i + phid->devChannelCnts.numTempInputs)) != NULL) {
					SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
					  "Input reported a potential of %.4fmV which is below the minimum of %.4fmV.", potential[i], phid->potentialMin);
					PhidgetRelease(&ch);
				}
			} else {
				phid->voltage[i] = potential[i];
			}
			break;
		case PHIDUID_1045:
		default:
	...
	for (i = 0; i < phid->devChannelCnts.numTempInputs; i++) {
		...
		if (tm < phid->temperatureDeadline[i])
			continue;

		if ((ch = getChannel(phid, i)) == NULL)
			continue;

		if (phid->lastTrigger[i] == PUNK_DBL ||
		  fabs(phid->temperature[i] - phid->lastTrigger[i]) >= phid->temperatureChangeTrigger[i]) {
			bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, "%g", phid->temperature[i]);
			phid->lastTrigger[i] = phid->temperature[i];
			phid->temperatureDeadline[i] = tm + phid->temperatureDataInterval[i];
		}
		PhidgetRelease(&ch);
	}
...
static PhidgetReturnCode CCONV
PhidgetTemperatureSensorDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetTemperatureSensor_ThermocoupleType t;
	PhidgetTemperatureSensorDeviceHandle phid;

	phid = (PhidgetTemperatureSensorDeviceHandle)ch->parent;

	MOS_ASSERT(phid->phid.deviceInfo.class == PHIDCLASS_TEMPERATURESENSOR);

	switch (ch->class) {
	case PHIDCHCLASS_TEMPERATURESENSOR:
		assert(ch->index < phid->devChannelCnts.numTempInputs);

		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			phid->temperatureDataInterval[ch->index] = HANDLE_DATAINTERVAL_PKT(bp, phid->interruptRate);
			phid->temperatureDeadline[ch->index] = 0;
			return (EPHIDGET_OK);
		case BP_SETCHANGETRIGGER:
			phid->temperatureChangeTrigger[ch->index] = getBridgePacketDouble(bp, 0);
			return (EPHIDGET_OK);
		case BP_SETTHERMOCOUPLETYPE:
			t = getBridgePacketInt32(bp, 0);
			phid->thermocoupleType[ch->index] = t;
			phid->maxTemperature[ch->index] = thermocouple_useful_range[t][1];
			phid->minTemperature[ch->index] = thermocouple_useful_range[t][0];
			/*
			 * Make sure our max and min range is actually measurable at the current ambient
			 * temperature. This is updated as the ambient temperature change in the data
			 * polling function.
			 */
			if (phid->temperature[phid->ambientSensorIndex] != PUNK_DBL) {
				if (lookup_voltage(phid->maxTemperature[ch->index] - phid->temperature[phid->ambientSensorIndex], t) > phid->potentialMax)
					phid->maxTemperature[ch->index] =
					(int)((lookup_temperature(phid->potentialMax, t) + phid->temperature[phid->ambientSensorIndex]) / 10) * 10;
				if (lookup_voltage(phid->minTemperature[ch->index] - phid->temperature[phid->ambientSensorIndex], t) < phid->potentialMin)
					phid->minTemperature[ch->index] =
					(int)((lookup_temperature(phid->potentialMin, t) + phid->temperature[phid->ambientSensorIndex]) / 10) * 10;
			}
			return (EPHIDGET_OK);
		case BP_OPENRESET:
		case BP_CLOSERESET:
			...
			case PHIDUID_1051_TERMINAL_BLOCKS:
			case PHIDUID_1048:
				if (ch->index != phid->ambientSensorIndex) {
					phid->thermocoupleType[ch->index] = PUNK_ENUM;
					phid->maxTemperature[ch->index] = PUNK_DBL;
					phid->minTemperature[ch->index] = PUNK_DBL;
				}
				break;
			case PHIDUID_1045:
			default:
	...
	case PHIDCHCLASS_VOLTAGEINPUT:
		...
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			phid->voltageDataInterval[ch->index] = HANDLE_DATAINTERVAL_PKT(bp, phid->interruptRate);
...
static const double thermocouple_table_e_type[THERMOCOUPLE_TABLE_E_SIZE] = {
...
static const double thermocouple_table_t_type[THERMOCOUPLE_TABLE_T_SIZE] = {
...
static const double thermocouple_table_j_type[THERMOCOUPLE_TABLE_J_SIZE] = {
...
static const double thermocouple_table_k_type[THERMOCOUPLE_TABLE_K_SIZE] = {
...
static double
lookup_temperature(double Vthermocouple, PhidgetTemperatureSensor_ThermocoupleType type) {
	...
	for (i = 1; i < size; i++) {
		if (thermocouple_table[i] > Vthermocouple) {
			temperature = (double)(i + (startingTemp - 1)) +
				((Vthermocouple - thermocouple_table[i - 1]) /
				(thermocouple_table[i] - thermocouple_table[i - 1]));
			break;
		}
	}
...
static double
lookup_voltage(double Tjunction, PhidgetTemperatureSensor_ThermocoupleType type) {
	const double *thermocouple_table;
	int startingTemp;
	double voltage;
	int size;
	...
	voltage = thermocouple_table[(int)Tjunction - startingTemp] +
		((thermocouple_table[(int)Tjunction - (startingTemp - 1)] -
			thermocouple_table[(int)Tjunction - startingTemp]) * (double)(Tjunction - (int)Tjunction));

	return (voltage / 1000.0);
}

static void CCONV
PhidgetTemperatureSensorDevice_free(PhidgetDeviceHandle *phid) {

	mos_free(*phid, sizeof(struct _PhidgetTemperatureSensorDevice));
	*phid = NULL;
}

PhidgetReturnCode
PhidgetTemperatureSensorDevice_create(PhidgetTemperatureSensorDeviceHandle *phidp) {
	DEVICECREATE_BODY(TemperatureSensorDevice, PHIDCLASS_TEMPERATURESENSOR);
	return (EPHIDGET_OK);
}
