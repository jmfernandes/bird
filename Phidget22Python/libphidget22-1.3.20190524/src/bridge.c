...
#ifdef DEBUG
#define bridgeloginfo(...) PhidgetLog_loge(NULL, 0, NULL, "_phidget22bridge", PHIDGET_LOG_INFO, __VA_ARGS__)
#else
...
#define ADJUSTPTRANDLEN do {						\
	...
	for (off = 0; off < bp->entrycnt; off++) {
		...
		case BPE_FLOAT:
		case BPE_DBL:
			w = snprintf(p, n, "{\"t\":\"%s\",\"v\":%e}", bridgePacketEntryType(bp->entry[off].type),
		...
		case BPE_DBLARRAY:
			...
			for (i = 0; i < bp->entry[off].bpe_cnt; i++) {
				if (i < bp->entry[off].bpe_cnt - 1)
					w = snprintf(p, n, "%e,", bp->entry[off].bpe_dblarray[i]);
				else
					w = snprintf(p, n, "%e", bp->entry[off].bpe_dblarray[i]);
				ADJUSTPTRANDLEN;
			}
	...
	*bridgePacket = (BridgePacket *)NULL;
	...
	reswitch:
		...
		case 'R':
			...
			if (n > 0)
				memcpy(bp->entry[bp->entrycnt].bpe_ui8array, va_arg(va, uint8_t *), n);
			break;
		case 'H':
			sz = n * sizeof(int16_t);
			...
			if (n > 0)
				memcpy(bp->entry[bp->entrycnt].bpe_i16array, va_arg(va, int16_t *), sz);
			break;
		case 'I':
			sz = n * sizeof(int32_t);
			if (sz > BPE_MAXARRAY_LEN) {
			...
			if (n > 0)
				memcpy(bp->entry[bp->entrycnt].bpe_i32array, va_arg(va, int32_t *), sz);
			break;
		case 'U':
			switch ((uint8_t)(*fmt)) {
			case 'H':
				...
				if (n > 0)
					memcpy(bp->entry[bp->entrycnt].bpe_ui16array, va_arg(va, uint16_t *), sz);
				fmt++;
				break;
			case 'L':
				...
				if (n > 0)
					memcpy(bp->entry[bp->entrycnt].bpe_ui64array, va_arg(va, uint64_t *), sz);
				fmt++;
				break;
			default:
				...
				if (n > 0)
					memcpy(bp->entry[bp->entrycnt].bpe_ui32array, va_arg(va, uint32_t *), sz);
				break;
			}
			break;
		case 'L':
			...
			if (n > 0)
				memcpy(bp->entry[bp->entrycnt].bpe_i64array, va_arg(va, int64_t *), sz);
			break;
		case 'G':
			sz = n * sizeof(double);
			if (sz > BPE_MAXARRAY_LEN) {
			...
			if (n > 0)
				memcpy(bp->entry[bp->entrycnt].bpe_dblarray, va_arg(va, double *), sz);
			break;
		case 'J':
			cptr = va_arg(va, const char *);
			n = (int)strlen(cptr);
...
void
freeBridgePacketEntry(BridgePacket *bp, int off, int freename) {

	if (freename && bp->entry[off].name) {
		mos_free(bp->entry[off].name, MOSM_FSTR);
		bp->entry[off].name = NULL;
	}

	switch (bp->entry[off].type) {
	case BPE_STR:
		mos_free(bp->entry[off].bpe_ptr, MOSM_FSTR);
		bp->entry[off].bpe_ptr = NULL;
		break;
	case BPE_JSON:
	case BPE_UI8ARRAY:
	case BPE_I16ARRAY:
	...
	case BPE_UI64ARRAY:
	case BPE_DBLARRAY:
		if (bp->entry[off].bpe_len != 0)
			mos_free(bp->entry[off].bpe_ptr, bp->entry[off].bpe_len);
		bp->entry[off].bpe_ptr = NULL;
		break;
	default:
		break;
...
destroyBridgePacket(BridgePacket **_bp) {
	...
	if (bp->reply != NULL)
		mos_free(bp->reply, MOSM_FSTR);

	for (i = 0; i < bp->entrycnt; i++)
		freeBridgePacketEntry(bp, i, 1);
...
networkSendBridgePacket(PhidgetChannelHandle channel, BridgePacket *bp, PhidgetNetConnHandle nc) {
	...
	res = simpleWaitForReply(&wfr, &rres, &reply);
	MOS_ASSERT(wfr == NULL);
	if (res != EPHIDGET_OK) {
		return (res);
	}
...
bridgeSendBPToDeviceWithReply(PhidgetChannelHandle channel, Phidget_AsyncCallback cb, void *ctx,
  BridgePacket *bp, char *reply, uint32_t replylen) {
	PhidgetReturnCode res;
	mosiop_t iop;
	...
	if (cb == NULL && reply && bp->reply != NULL)
		mos_strlcpy(reply, bp->reply, replylen);
...
bridgeSendBPToDevice(PhidgetChannelHandle channel, Phidget_AsyncCallback cb, void *ctx, BridgePacket *bp) {
	...
	return (bridgeSendBPToDeviceWithReply(channel, cb, ctx, bp, NULL, 0));
}

PhidgetReturnCode
bridgeSendToDeviceWithReply(PhidgetChannelHandle channel, bridgepacket_t pkt, Phidget_AsyncCallback cb,
  void *ctx, char *reply, uint32_t replylen, const char *fmt, ...) {
	PhidgetReturnCode res;
	BridgePacket *bp;
	va_list va;
