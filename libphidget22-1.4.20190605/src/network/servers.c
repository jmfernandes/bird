...
deviceServerListener(ZeroconfListenerHandle hdl, void *ctx, int added, int interface, Zeroconf_Protocol proto,
  const char *name, const char *host, const char *type, const char *domain) {
	...
	kv = NULL;
	if (serverAdded)
		res = Zeroconf_lookup(hdl, interface, proto, name, host, type, domain, ZCP_IPv4, &addrlist, &port, &kv);
	else
		res = Zeroconf_lookup(hdl, interface, proto, name, host, type, domain, ZCP_IPv4, &addrlist, &port, NULL);
	if (res != 0) {
		netlogerr("Zeroconf_lookup() failed for %s [%s]", name, host);
		return;
	}

	netloginfo("Discovered server '%s' [%s/%s] (interface 0x%x)", name, host,
	  mos_getaddrinfo(&addrlist->addr, NULL, 0), interface);

	res = PhidgetNet_discoveredServer(hdl, 0, interface, PHIDGETSERVER_DEVICEREMOTE, name, host, type,
	  domain, port);
	if (res != 0)
...
checkKeepAlive(PhidgetNetConnHandle nc) {
	...
	case PHIDGETCONN_LOCAL:		/* Server */
		...
		if (nc->keepalive_dl != 0 && nc->keepalive_dl < tm) {
			netlogerr("%"PRIphid": keepalive [%"PRId64" usec] timeout [%"PRId64" usec < %"PRId64" usec]",
			  nc, nc->keepalive, nc->keepalive_dl, tm);
...
readcomplete:
	...
	res = nc->handleRequest(iop, nc, &req, stop);
	if (res != EPHIDGET_OK)
		netlogerr("%"PRIphid": msgtype: %s/%s failed: "PRC_FMT"\n\t%N",
		  nc, strmsgtype(req.nr_type), strmsgsubtype(req.nr_stype), PRC_ARGS(res), iop);
