...
PhidgetVINTDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case BP_CLOSERESET:
		...
		res = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_PHIDGET_RESET, NULL, 0, NULL);
		if (res != EPHIDGET_OK) {
	...
	case BP_ENABLE:
		res = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_PHIDGET_ENABLE, NULL, 0, NULL);
		if (res != EPHIDGET_OK) {
