...
#define VINT_PHIDGET(uid, did, id) PHIDTYPE_VINT, did ,uid, PHIDCLASS_VINT, {.dummy = { 0 }}, 0, 0, 0, id
#define VIRTUAL_PHIDGET(uid, did, cls, id) PHIDTYPE_VIRTUAL, did, uid, cls, {.dummy = { 0 }}, id, 0, 0, 0
#else
#define ATTR(name, ...) { __VA_ARGS__ }
#define VINT_PHIDGET(uid, did, id) PHIDTYPE_VINT, did, uid, PHIDCLASS_VINT, {0}, 0, 0, 0, id
#define VIRTUAL_PHIDGET(uid, did, cls, id) PHIDTYPE_VIRTUAL, did, uid, cls, {0}, id, 0, 0, 0
#endif

#define USB_PHIDGET(uid, did, usbclass, vid, pid, interface, attr, ...) PHIDTYPE_USB, did, uid, usbclass, ATTR(attr, __VA_ARGS__), pid, vid, interface, 0
#define MESH_PHIDGET(uid, did, meshclass, id, attr, ...) PHIDTYPE_MESH, did, uid, meshclass, ATTR(attr, __VA_ARGS__), id, 0, 0, 0
#define SPI_PHIDGET(uid, did, spiclass, id, attr, ...) PHIDTYPE_SPI, did, uid, spiclass, ATTR(attr, __VA_ARGS__), id, 0, 0, 0
#define LIGHTNING_PHIDGET(uid, did, usbclass, vid, pid, interface, attr, ...) PHIDTYPE_LIGHTNING, did, uid, usbclass, ATTR(attr, __VA_ARGS__), pid, vid, interface, 0
...
PhidgetReturnCode
deviceBridgeInput(PhidgetChannelHandle ch, BridgePacket *bp, int lock) {
